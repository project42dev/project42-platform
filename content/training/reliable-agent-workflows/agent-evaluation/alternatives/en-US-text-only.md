# Evaluate Agent Behavior with Validated Evidence: text-only class
...
Visual alternative: The lesson moves from a testable claim through cases, evidence validation, scoring, comparison, release decision, and regression maintenance.
...
Visual alternative: Each case has a stable identifier and slice plus versioned input, expected result, permitted trajectory, prohibited behavior, scoring, budgets, and provenance.
...
Visual alternative: The learner records a claim, a must-not-regress slice, a threshold, and a trajectory failure.
...
Visual alternative: Ten stable cases cover representative, boundary, adversarial, and regression slices.
...
Visual alternative: Outcome, trajectory, policy, cost, and latency are scored separately, with deterministic, human, and model evidence kept distinct.
...
Visual alternative: The candidate averages eighty-nine point sixty, but ADV-01 has an unauthorized trajectory, policy zero, and a critical failure.
...
Visual alternative: The candidate improves the fixture mean but is held because one critical authorization case fails.
...
Visual alternative: A higher average does not override a failed critical permission case.
...
Visual alternative: A critical failure requires investigation and retesting before ship or rollback.

If correct: You preserved the predeclared critical gate instead of rationalizing an unsafe average.
...
Visual alternative: Incident evidence is minimized, reviewed, versioned, added to a slice, and kept reproducible without unnecessary sensitive content.
...
Visual alternative: The learner opens the supplied activity, verifies ten cases and gates, and edits only the gate file.
...
Visual alternative: Malformed evidence is rejected, the starter incorrectly ships a critical failure, and the repaired gate holds it.
...
Visual alternative: Recovery changes only the unsafe case and ships at ninety-one point sixty, while a ninety-five point eighty overall score still holds for a seventy-nine point zero zero adversarial slice.
...
Visual alternative: The learner can review the class, return to the activity, or begin the eight-question knowledge check.
...
Visual alternative: The closing rules are to predeclare, cover risk, validate evidence, score trajectories, compare fairly, honor gates, and preserve provenance.
