# Engineer Context for Reliable Decisions: text-only class
...
Visual alternative: Policy, goal, state, evidence, untrusted data, prior decisions, tools, output requirements, budget, and missing information occupy separate rows with provenance.
...
Visual alternative: Each candidate receives one outcome: include, summarize with provenance, retrieve later on a trigger, or reject.
...
Visual alternative: Current primary sources are included, duplicate and stale material is rejected or deferred, injected instructions remain untrusted, and contradiction stays visible.
...
Visual alternative: Instructions, current request, tool schemas, response, and tool results are reserved; evidence is ranked; duplicate and dead history is removed first.
...
Visual alternative: Messages, stored responses, compaction, and caches transport context; the application refreshes authoritative state and owns provenance.
...
Visual alternative: Injection, stale facts, contradiction, omission, overload, poisoned summary, and unnecessary sensitive data each have an explicit safe outcome.
...
Learner action: Build a role-separated context manifest for one bounded decision.
...
Learner action: Keep any supported factual evidence with provenance, label the article untrusted, and reject the instruction as authority.
...
If correct: You preserved useful evidence without allowing untrusted text to expand authority.

If retrying: Classify each passage by role: evidence may inform the decision, but only trusted policy and user intent define authority.
