...
Visual alternative: The packet contains schema, trace, sender, recipient, reason, goal, constraints, facts, artifacts, actions, permissions, budget, success, and return states.
...
Visual alternative: Only required facts and artifacts pass the context filter. The recipient receives only actions allowed by its policy and not denied by the packet.
...
If retrying: Grant only actions and objects required by the reviewer's assigned task.
...
Visual alternative: Validated packets may be accepted or rejected. Accepted work returns a typed result. Zero work turns pauses, while rejection and ambiguous timeout follow separate paths.
...
Visual alternative: The validation gate checks structure, provenance, consistency, recipient, event, depth, and budget. The authorization gate calculates effective actions.
...
Visual alternative: The packet requests read and publish, policy allows read and comment, denied actions include publish and delete, and the result has read and two remaining turns.
...
Visual alternative: Correct transfer and narrowed excessive permission are accepted. Wrong recipient, missing provenance, poisoned context, and recursive bounce are rejected. Timeout and receiver rejection escalate.
...
Visual alternative: The starter has twenty learner passes out of twenty-two because sender actions are copied. The repair produces twenty-two out of twenty-two and pass.
...
Learner action: Answer that the effective action set is comment and show intersection followed by denied-action subtraction.
...
If correct: You preserved comment through the recipient-policy intersection and removed denied delete.

If retrying: Recompute the intersection first. Do not use the denied set as the only authorization rule.
...
Visual alternative: Local recovery reruns tests after restoring the learner file. Runtime recovery preserves the ledger and escalates after ambiguous timeout.
...
Visual alternative: Model family, runtime or API, and agent adapter are separate from application validation, authorization, budgets, and idempotency.
...
Visual alternative: Every rubric claim requires a concrete artifact or trace, and fixture results must not be presented as live provider evidence.
