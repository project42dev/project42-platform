...
Package: `prepare-agent-work-class` 1.0.0
...
Expected learner action: Define success, approval, no-progress, budget, and unknown-outcome evidence for one bounded run.
...
Expected learner action: Stop for repeated non-progress and preserve the evidence gap instead of spending the remaining budget.
...
Correct feedback: You stopped a no-progress cycle and preserved evidence for a materially different recovery or useful handoff.

Retry feedback: Compare evidence before and after each action. Remaining budget cannot justify an equivalent transition that produced no progress.
