...
Expected learner action: Compare all three recorded predictions with the actual fixture results, mark each as correct or revise it, and identify the first failed control for every case. Continue only after the learner has accounted for all three terminal outcomes and confirmed that each pre-effect block has zero effects.
