...
Visual alternative: The fixed workflow uses code-defined checks and one write. The model-directed loop may propose a next action for necessary variation, but the controller still owns authority, budgets, execution, evidence, terminals, and recovery.
...
Visual alternative: The card defines one receipt, in-memory scope, trusted controller inputs, no external effects, independent verification, and five terminals.
...
Visual alternative: Policy, limits, state, verifier, and approvals are trusted controller inputs. Observations and decisions can propose but cannot grant authority.
...
Visual alternative: The sequence is inspect, decide, authorize, preflight, act, verify, and record. A successful write moves revision one to revision two and requires verifier acceptance.
...
Visual alternative: COMPLETE requires verified evidence. NEEDS_HUMAN follows three no-progress transitions. BUDGET_EXHAUSTED blocks preflight overspend. POLICY_BLOCKED rejects malformed or unauthorized work before effects. FAILED_WITH_RECOVERY_EVIDENCE preserves an unknown consequential outcome.
...
Learner action: Predict BUDGET_EXHAUSTED for policy-only, POLICY_BLOCKED for budget-only, and POLICY_BLOCKED with zero effects for stale approval.
...
If correct: You distinguished policy permission, action capacity, and exact approval binding.
...
Visual alternative: The uncertain write may have created a receipt. Recovery finds one effect by operation key and performs no retry while preserving FAILED_WITH_RECOVERY_EVIDENCE.
...
If correct: Your evidence distinguishes independent completion, no-progress, preflight budget exhaustion, policy denial, and uncertain-outcome reconciliation.

If retrying: Return to the first failed controller check and the environment effect count. Do not infer authority or completion from decision text.
