...
Package: `reliable-agent-capstone-class` 2.0.0
...
Expected learner action: Identify transitions, evidence, stop conditions, and the sole authority for effects.
...
Correct feedback: You checked the workflow as a state and evidence contract rather than as a model demonstration.

Retry feedback: Name one allowed transition, its durable record, its stop condition, and the component allowed to authorize it.
...
Expected learner action: Explain that valid revised and changed packages must be accepted while stale evidence is rejected.
...
Correct feedback: You distinguished least authority from a validator that simply rejects everything.

Retry feedback: State which valid packages must pass and which stale package must fail.
...
Expected learner action: Choose and explain that a handoff is a contract with evidence and provenance.
...
Correct feedback: You treated provenance and acceptance checks as part of the handoff contract.

Retry feedback: Name the four binding values the receiver must compare with the manifest.
...
Expected learner action: Explain that a failure test supplies evidence of detection, containment, reconciliation, or recovery.
...
Correct feedback: You separated a possibility demonstration from evidence that controls work under failure.

Retry feedback: Choose one failure and state the expected containment and the evidence that would prove it.
...
Expected learner action: Describe telemetry that explains what happened and what to do next without becoming a data leak.
...
Correct feedback: You connected observability to recovery while preserving privacy boundaries.

Retry feedback: Name two safe operational fields and two categories of content that should be excluded.
...
Expected learner action: Map one weak criterion to concrete missing evidence and a bounded revision.
...
Retry feedback: For every awarded point, identify the stable artifact or assessment result another reviewer can inspect.
...
Expected learner action: Predict architecture-and-state-model.md and attemptId, then explain the provenance gap.
...
Correct feedback: You identified the causal provenance mismatch rather than treating file existence as evidence.

Retry feedback: Compare the first artifact binding with the manifest and name the first unequal value.
...
Expected learner action: State that tests pass, failed evidence remains preserved, revised and changed inputs pass, and the handoff separates offline validation from live or human qualification.
...
Correct feedback: You included both technical acceptance and the boundary around what the evidence does not establish.

Retry feedback: List the passing fixtures, the preserved history, and the live or human claims that remain unqualified.
