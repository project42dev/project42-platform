...
Package: `orchestration-patterns-class` 1.0.0
...
Expected learner action: Select the smallest justified pattern and document why each retained agent boundary exists.
...
Expected learner action: Reject default parallel execution because shared mutation requires coordination or serialization.
...
Correct feedback: You recognized that independence is a data and side-effect property, not a model property.

Retry feedback: Check whether either branch depends on or mutates state used by the other.
