...
Package: `operate-and-recover-agent-systems-class` 2.0.0
...
Expected learner action: Treat the result as unknown and reconcile authoritative state before another mutation.
...
Correct feedback: You identified the unknown postcondition and protected the external system from duplicate impact.

Retry feedback: A timeout describes the response path, not the final state. Reconcile the system of record before another mutation.
...
Expected learner action: Predict the changed classifications and identify the required missing-write assertion.
...
Correct feedback: You separated persisted confirmation, absent state, and unverifiable pending state, then connected each to its distinct recovery action.

Retry feedback: Use the changed ledger, not the baseline names. The absent audit record is the evidence that deny-all cannot satisfy the repair.
