...
Package: `control-agent-actions-class` 1.0.0
...
Expected learner action: Select object-level authorization and explain why schema validity, target existence, approval, and model recommendation do not grant authority.
...
Correct feedback: Correct. You identified the first trusted control and connected it to the prevented mutation.

Retry feedback: Start with the resolved target and current caller, then move through revision, approval, budget, mutation, and independent evidence.
...
Expected learner action: Edit only the three incorrect approval bindings and run the four independent exercise checks.
...
Correct feedback: The three approval fields now bind the exact target, revision, and operation key while negative variants remain unable to mutate.

Retry feedback: Compare the action with approval.resourceId, approval.expectedVersion, and approval.operationKey. Do not broaden actor scope or policy.
