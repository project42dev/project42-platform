...
Package: `mcp-architecture-class` 1.0.0
...
Expected learner action: Create a two-server architecture that preserves connection, capability, data, and approval boundaries.
...
Expected learner action: Do not call an operation that was not negotiated; use the declared fallback or fail closed.
...
Correct feedback: You treated negotiated capability as runtime evidence and followed a declared fallback.

Retry feedback: Compare desired configuration with the capability state actually agreed for the current session.
