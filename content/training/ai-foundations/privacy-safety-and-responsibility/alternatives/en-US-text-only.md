...
Visual alternative: The gate asks what the data is, whether its use is authorized, which product is approved, what is retained, who can access it, whether it is necessary, and what can be removed.
...
Visual alternative: The original contains identity, accommodation, salary, customer detail, and a token. The revised brief keeps observable work facts, generalizes the customer, removes personal data, and excludes the secret.
...
Learner action: Classify a prospective input, minimize it to the task, and record unresolved authority or policy as a stop condition.
...
Visual alternative: Private ideation has low consequence and high reversibility. Applicant ranking has high consequence, wider exposure, sensitive data, fairness risks, and a need for qualified oversight and correction.
...
Visual alternative: Privacy covers data handling; security covers unauthorized access and abuse; safety covers harm; fairness covers differing performance and impact; transparency covers disclosure and correction; accountability covers ownership.
...
Learner action: Identify transparency and accountability as weak, while noting that fairness and outcome quality still require separate evidence.
...
If correct: You separated strong security controls from missing transparency, recourse, and accountable ownership.

If retrying: Ask who explains the AI role, who corrects an error, and who owns the decision. The workflow provides no answer.
...
Learner action: Decide independently: launch, assistance-only redesign, or manual process. Explain how consequence, exposure, and reversibility affect your decision. State which of the proposed inputs you would omit and why, the minimum data that could be considered for a later approved use, what the system may and may not do, who must authorize the use, which product terms or evidence are missing, and the first stop or recovery action if processing occurs without approval or if an output has already affected an applicant. Do not reveal or use an answer key before submitting.
