# Context and Evidence Construction: text-only class
...
Visual alternative: Documents pass through five explicit selection criteria. The final set contains only sources that can support the defined decision, while exclusions and gaps remain visible.
...
Visual alternative: Current official documentation and policy are included, pricing is excluded, an old slide is recorded as a conflict, and a forum post is only a research lead.
...
Visual alternative: Each source has publisher, date, location, supported claims, scope limits, permission status, and a rule for conflicts.
...
Learner action: Create one source record containing publisher, date, supported claim, limitation, permission status, and visible unknowns.
...
Visual alternative: Documents and user text can provide evidence but cannot change permissions, tool policy, secrets access, or the task's authorized instructions.
...
Learner action: Classify the command as untrusted source content, reject its requested authority and data disclosure, and use only separately verified diagnostic facts within the authorized task.
...
If correct: You preserved the instruction boundary, denied secret access and transmission, and limited useful facts to verified evidence inside trusted policy.

If retrying: Separate claim authority from instruction authority: retrieved text may be evidence, but it cannot grant permissions or redefine the task.
...
Visual alternative: Every claim maps to a source passage and support status. Inference and conflict are separate fields, and unsupported claims remain visible gaps.
...
Visual alternative: Feature documentation proves the feature exists, and a regional table proves three regions. Neither proves worldwide availability, so the global claim is recorded as an evidence gap.
