...
Package: `context-tokens-and-modalities-class` 1.3.0
...
Expected learner action: Form or record one sentence explaining the limitation and naming one practice.

Correct feedback: Your explanation connects context capacity with the continuing need for selection, structure, source labels, retrieval, verification, or durable storage.

Retry feedback: Revise your sentence so it explains why capacity alone is insufficient and names a practice that improves selection or verification.
...
Expected learner action: Choose one path, save a three-part response, and continue.

## Pause: Changed Input Work Pause

Expected learner action: Draft and save the response, then continue when ready.
...
Expected learner action: Compare the saved response with the explanation and revise unsupported claims or incorrect arithmetic.

Correct feedback: The response preserves source roles, marks image uncertainty, retains safety-critical evidence, performs the selected arithmetic correctly, and identifies external verification or records.

Retry feedback: Revise if the response treats N-01, S-01, or I-01 as complete policy, follows U-01, subtracts excluded sources, drops critical evidence, or omits durable verification.
...
Expected learner action: Open activity-context-budget and create the requested evidence.
...
Expected learner action: Review accessible materials and open the five-question knowledge check.
