...
Package: `agents-and-guardrails-class` 1.0.0
...
Expected learner action: Complete every run-contract field and identify action, ticket, tenant, version, and value as the five approval fields.
...
Expected learner action: Stop repeated non-progress, preserve the trajectory and evidence gap, and hand off or use only an authorized materially different recovery.
...
Expected learner action: Save a complete trace and state prediction before continuing to the answer.
...
Correct feedback: You predicted stale approval, exactly two spent units, unchanged balance, open status, version four, and an authorized handoff.

Retry feedback: Follow precheck order. Trusted version four differs from proposed version three before permission is evaluated, and no action charge occurs.
