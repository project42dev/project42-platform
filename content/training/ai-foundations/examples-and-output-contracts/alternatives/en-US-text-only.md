...
Visual alternative: The rule appears first. Normal, boundary, and missing-information examples show different applications of the rule.
...
Visual alternative: The cases are labelled access, billing, conflict, and missing evidence, and each label names the rule it exercises.
...
Learner action: Identify three varied examples and state the rule each example exercises.
...
Visual alternative: The template lists purpose, format, required fields, allowed values, evidence, unknown handling, ordering, limits, and validation.
...
Learner action: Draft a provider-neutral contract with purpose, consumer, fields, allowed values, evidence, and unknown handling.
...
Visual alternative: Transport, parsing, schema, business, evidence, and authorization are separate checks.
...
If retrying: Name each layer independently. A structured response cannot create evidence or grant authority.
...
Visual alternative: Normal, boundary, missing, conflict, refusal, truncation, and malicious cases have explicit validation or review paths.
...
Visual alternative: Field and enum checks pass, but the evidence conflicts with the category and the requested action is unauthorized.
...
Learner action: Submit one contract-compliant answer for I-001 before viewing feedback.
...
Learner action: State the difference between examples, contracts, structural validation, content validation, and authorization.
