...
Package: `scaling-failure-and-capacity-controls-class` 2.0.0
...
Expected learner action: Identify multiplicative retry and queue amplification, then specify end-to-end deadlines, cancellation, bounded classified retries, a retry budget, jitter, a circuit, bounded queues, shedding, and safe recovery.
...
Expected learner action: Calculate ready capacity, queue coverage, deadline, failure domains, and cost, then predict ADMIT and APPROVE before execution.
...
Correct feedback: You checked all four gates and predicted approval from ready capacity, queue coverage, deadline, domains, and cost.

Retry feedback: Recalculate with ready non-draining slots, then check queue, deadline, domains, and cost separately. Do not use nominal capacity.
...
Expected learner action: Complete the comparison, policy, lifecycle, failure matrix, traffic calculations, prediction, repair, changed-input explanation, and reflection.
