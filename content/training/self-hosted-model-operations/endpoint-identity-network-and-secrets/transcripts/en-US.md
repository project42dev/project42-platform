...
Package: `endpoint-identity-network-and-secrets-class` 2.0.0
...
Expected learner action: Predict denial with reason scope_mismatch, based on the validated principal tenant and resource tenant, with no unauthorized side effect.
...
Correct feedback: You compared the validated principal and resource scope outside the model and denied the cross-tenant object.

Retry feedback: Use trusted identity and object authorization. Prompt text, model output, and role alone cannot establish permission.
...
Expected learner action: Predict deny with reason scope_mismatch because tenant-blue differs from tenant-red, despite valid authentication, role, and units.
...
Correct feedback: You applied the tenant comparison to the independent changed case and preserved valid same-tenant behavior.

Retry feedback: Do not special-case names or deny everything. Compare validated principal and resource scope, then preserve the existing valid path.
...
Expected learner action: Complete the threat model, least-privilege matrix, control matrix, negative-test and recovery report, exact lab evidence, independent prediction, changed-input answer, causal explanation, and residual-risk labels.
