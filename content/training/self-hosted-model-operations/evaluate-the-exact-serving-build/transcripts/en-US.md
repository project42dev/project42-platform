...
Package: `evaluate-the-exact-serving-build-class` 2.0.0
...
Expected learner action: Choose reject or hold, identify the unauthorized write as a critical prohibited effect, preserve deterministic side-effect evidence and grader disagreement, and require authorized human adjudication.
...
Correct feedback: Correct. You kept the critical authorization failure blocking and preserved the evidence for human investigation.

Retry feedback: Do not average away an unauthorized effect. Identify the critical gate, preserve the side-effect record, and require authorized human disposition.
...
Expected learner action: Record predictions for the starter, repaired candidate, and all-pass changed dossier before viewing the answer.
...
Correct feedback: Correct if you identified independent fail-closed conditions and rejected filename checks or deny-all logic.

Retry feedback: The repair must block each failed condition while accepting a valid changed all-pass dossier. Do not hard-code a case or reject every input.
...
Expected learner action: Answer the changed valid case separately from the original candidate and explain why content-based evaluation must accept it.
...
Correct feedback: Correct. You separated the changed valid all-pass result from the original C06 failure and rejected deny-all logic.

Retry feedback: The positive control is 12/12, 100.00 percent, zero critical failures, PASS, and exit code 0. Its filename must not matter.
...
Expected learner action: Complete the versioned claims, governed cases, grader assignments, baseline-candidate comparison, changed-input reasoning, critical-failure handling, and evidence-linked human disposition.
