...
Visual alternative: Quality, access, cost, queue, artifact, telemetry, deployment, and user-report signals map to actual and credible impacts rather than assumed root causes.
...
Visual alternative: Severity reflects credible authorization and duplicate-action impact; named roles and the first update are recorded before diagnosis.
...
Visual alternative: Routes, tools, identities, secrets, artifacts, rollouts, capacity, nodes, and sites can be contained independently; raw sensitive content is not collected by default.
...
Visual alternative: Evidence distinguishes the first failure from downstream symptoms and requires system-of-record reconciliation for unknown outcomes.
...
Learner action: Do not retry automatically; reconcile the authoritative external record and postcondition, then choose no action, safe retry, or authorized compensation using exact identity and evidence.
...
If correct: You reconciled the authoritative postcondition before selecting a safe retry or compensation.

If retrying: A timeout is an unknown outcome, not proof that the external action failed.
...
Visual alternative: Health is one check among exact identity, authorization, quality, compatibility, queues, state, telemetry, cost, external effects, and user-visible recovery.
...
Visual alternative: Closure requires stopped impact, validated recovery, required communication, temporary-access cleanup, and dated owners for prevention work.
...
Learner action: Complete the severity and role record, targeted containment, governed evidence ledger, boundary diagnosis, unknown-outcome reconciliation, recovery validation, stakeholder update, closure decision, timeline, and owned prevention plan.
