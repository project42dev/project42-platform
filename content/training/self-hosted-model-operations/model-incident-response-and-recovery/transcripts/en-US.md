...
Package: `model-incident-response-and-recovery-class` 2.0.0
...
Expected learner action: Choose HOLD_RECONCILE and identify the authoritative final record and observed postcondition required before retry or compensation.
...
Correct feedback: You selected HOLD_RECONCILE and required matching authoritative final evidence before another side effect.

Retry feedback: A timeout is uncertainty, not failure. Require matching authoritative evidence and an observed postcondition before acting.
...
Expected learner action: Complete the incident exercise and submit the decision record, timeline, communication, closure evidence, prevention backlog, predictions, repair, and causal explanation.
...
Expected learner action: Record predictions for all listed evidence and recovery variations before continuing.
...
Expected learner action: Submit the timeline, closure conditions, and owned prevention backlog without claiming real execution or approval.
