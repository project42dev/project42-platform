# Choose a Deployment Shape and Operating Model: text-only class
...
Visual alternative: The class starts with a workload contract and compares six deployment shapes using evidence about ownership, failures, recovery, and reversibility.
...
Visual alternative: The deployment decision begins with users, tasks, data, demand, latency, availability, failure behavior, retention, and recovery rather than a product choice.
...
Visual alternative: The worked workload serves 120 analysts, handles 60 requests per minute with bursts to 120, keeps confidential text in-country, and has explicit latency, availability, retention, and recovery objectives.
...
Visual alternative: Each deployment shape has different tradeoffs, and none automatically proves privacy, cost, performance, availability, or resilience.
...
Visual alternative: The matrix gives scores of 18, 64, 78, 55, 92, and 73, but only on-premises service and hybrid satisfy all mandatory constraints.
...
Learner action: Identify one platform bias, one unproven property, and one concrete measurement, review, or test.
...
Visual alternative: Replicas protect against some process failures but shared host and control dependencies can remove every replica or block recovery.
...
Learner action: Name two shared failures and require an observed recovery test outside the failed host.
...
If correct: You kept the availability claim inside the tested boundary and required observed recovery outside the shared host.

If retrying: Identify a dependency shared by every container, then require recovery capacity and an observed test outside that dependency.
...
Visual alternative: Each failure has a specific response, but the fixture does not perform live failure tests or prove recovery objectives.
...
Visual alternative: The evaluation has qualified coverage for named responsibilities but lacks a qualified artifact-custodian substitute and after-hours incident command.
...
Visual alternative: The first shape is a staffed on-premises evaluation selected after constraints and ranking, with a tested safe return and explicit limits on the claim.
...
Learner action: Predict on-premises-service for baseline and cloud-service for changed evidence, with an eligibility-before-ranking explanation.
...
Visual alternative: The lab repairs eligibility-first selection, preserving validation and scoring, then verifies on-premises for baseline and cloud for changed evidence.
...
If correct: You applied eligibility before ranking, treated unknown evidence as disqualifying for mandatory constraints, and explained why changed valid evidence selects cloud-service.

If retrying: Separate incomplete evidence from satisfied constraints. Then filter eligible options before comparing scores, including the changed boundary, recovery, and owner evidence.
...
Visual alternative: Define the work and failure behavior, assign ownership, apply constraints before ranking, choose a reversible shape, and stay within the evidence.
