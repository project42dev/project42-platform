# Model Identity, Licensing, and Provenance: text-only class
...
Visual alternative: Rows identify publisher, repository, revision, files, byte count, digests, base model, derivative, quantization, tokenizer, template, runtime, architecture, and deployment shape; unknown fields remain explicitly unresolved.
...
Visual alternative: The example contains several publishers and transformations. The base revision and conversion recipe are unknown, so the identity claim is incomplete.
...
Visual alternative: Each label requires different evidence. Download access alone does not prove permission to modify, redistribute, or call the complete system Open Source AI.
...
If correct: You limited the claim to observed access and kept redistribution and Open Source AI status pending exact evidence.
...
Visual alternative: Every transformation links to evidence. Missing or contradictory links remain visible and lead to stop, request, alternate, or bounded-review decisions.
...
Visual alternative: The weights bytes are abc, the tokenizer is empty, both hashes are shown exactly, completeness is twelve of twelve, and the result is fictional and non-production.
...
Visual alternative: The panel lists the exact lab directory, Node.js 22 requirement, starter test, learner test, and reference test; no command runs automatically.
...
Learner action: Complete the identity tuple, three-row terms matrix, lineage map, evidence labels, bounded disposition, reviewers, expiry, and re-review triggers.
...
Learner action: Answer hold-for-evidence, identify unknown external API distribution as the cause, and state that stale proposed values must be rejected.
...
If correct: You held the changed scope because its external API distribution lacks permission evidence and bound every matrix row to the current proposal.

If retrying: A matching digest proves byte identity only. Recheck the changed distribution and the required proposedValue equality.
