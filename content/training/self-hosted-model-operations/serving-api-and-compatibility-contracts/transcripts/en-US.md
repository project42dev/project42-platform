...
Correct feedback: You withheld routing until exact runtime and model identity evidence was available.

Retry feedback: A live port and alias do not prove the pinned artifacts. Name the hashes and revisions required before routing.
...
Correct feedback: You separated client timeout, later recovery, and unobserved server cancellation.

Retry feedback: A client deadline is not server cancellation. State the missing server-owned evidence.
...
Correct feedback: You distinguished harmless wire-field differences from loss of a required semantic field and did not equate offline eligibility with authorization.

Retry feedback: Dropping explanation is a user-visible contract failure. The alias is not artifact proof, and offline eligibility is not deployment authorization.
...
Correct feedback: You enforced actual Boolean typing, derived escalation from the permitted category, and preserved the four-field normalized output.

Retry feedback: Integer 1 is not an actual Boolean here. The adapter must derive the transport label and reject caller-supplied ambiguity.
...
Correct feedback: You refused to stop an unrelated process and required fresh ownership evidence before recovery.

Retry feedback: The port does not establish ownership. Compare PID, executable path, and startTimeUtc, and preserve the refusal evidence.
...
Expected learner action: Run the pinned service and exercises, preserve receipts and failures, classify known and unknown behavior, implement the changed adapter in the practice copy, and produce reversible cutover evidence.
...
Expected learner action: Review the evidence bundle, correct unsupported claims, and choose Begin knowledge check only after the practical work is complete.
