...
const scratch = new URL('test-scratch/', root);
const bridge = new URL('src/bridge.mjs', root);
const starter = `import {validateProposal, validateStores, publicFailure} from './contracts.mjs';
...
await writeFile(new URL('.gitkeep', scratch), '');
await writeFile(bridge, starter);
console.log('RESET lab starter and test-scratch');
