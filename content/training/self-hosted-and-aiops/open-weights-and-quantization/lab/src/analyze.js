...
function requireFinitePositive(value, path) {
  if (typeof value !== "number" || !Number.isFinite(value) || value <= 0) {
    throw new TypeError(`${path} must be a finite number > 0`);
  }
}
...
export function validateVariant(variant, path = "variant") {
  ...
  if (variant.runtime.mode === "dequantized") {
    requireInteger(variant.runtime.denseBits, `${path}.runtime.denseBits`, 1);
  } else if (variant.runtime.denseBits !== null) {
    throw new TypeError(`${path}.runtime.denseBits must be null unless mode is dequantized`);
  }
  ...
  if (variant.quality.score > variant.quality.maximum || variant.quality.threshold > variant.quality.maximum) {
    throw new RangeError(`${path}.quality score and threshold must not exceed maximum`);
  }
  ...
  for (const key of ["artifact", "lineage", "runtime", "quality"]) {
    requireString(variant.evidence[key], `${path}.evidence.${key}`);
  }
  return variant;
}
...
export function predictedPayloadBytes(variant) {
  ...
  if (!Number.isSafeInteger(bytes)) {
    throw new RangeError(`${variant.id} predicted payload must be an exact safe integer byte count`);
  }
  return bytes;
}

// DELIBERATE DEFECT: this assumes every runtime consumes packed weights.
// Repair this function only. See README.md for the required evidence gate.
...
export function analyzeVariant(variant) {
  ...
  return {
    id: variant.id,
    predictedPayloadBytes: payloadBytes,
    actualFileBytes: variant.actualFileBytes,
    overheadBytes,
    discrepancyPercent,
    residentWeightBytes,
    memoryBudgetBytes,
